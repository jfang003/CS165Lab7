...
//	SSL_CTX_set_options(ctx, SSL_OP_ALL | SSL_OP_NO_SSLv2 | SSL_OP_SINGLE_DH_USE);
  ...
  string challenge=buff;
	...
	if(BIO_write(mem, challenge.c_str(), challenge.size())<=0)//BIO_write
		printf("ERROR.\n");
	...
	string hash_string = buff2hex((unsigned char *)mdbuf,challenge.size());

	printf("SUCCESS.\n");
	printf("    (SHA1 hash: \"%s\" (%d bytes))\n", hash_string.c_str(), hash_string.size());
	...
	RSA* priv = PEM_read_bio_RSAPrivateKey(rsaprivate, NULL, 0, NULL);//PEM_read_bio_RSAPrivateKey
	unsigned char* sig = (unsigned char*) malloc(RSA_size(priv));
	memset(sig, 0, RSA_size(priv));
	//RSA_private_encrypt
	int ret = RSA_private_encrypt(challenge.size(), (unsigned char*) mdbuf, sig, priv, RSA_PKCS1_PADDING);

    int siglen=ret;
    char* signature=(char*)sig;
	...
	BIO_flush(server); //BIO_flush
	buff_len = SSL_write(ssl, sig, 128);//SSL_write
    ...
    memset(file,0,sizeof(file));
		buff_len = SSL_read(ssl, file, BUFFER_SIZE);
    printf("RECEIVED.\n");
	...
	while(shutdown != 1)
	{
		if(shutdown==-1)
		{
			printf("Error while shutting down(ssl).\n");
      print_errors();
			exit(EXIT_FAILURE);
		}
