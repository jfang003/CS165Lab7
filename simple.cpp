...
int main(int argc, char *argv[])
{
	...
	RSA* priv = PEM_read_bio_RSAPrivateKey(rsaprivate, NULL, 0, NULL);
	...
	unsigned char* decrypt = (unsigned char*) malloc(RSA_size(priv));
	int ret = RSA_private_encrypt(EVP_MAX_MD_SIZE, (unsigned char*) mdbuf, sig, priv, RSA_PKCS1_PADDING);
	for(int i=0;i<EVP_MAX_MD_SIZE;i++)
		printf("%02x", sig[i] & 0xFF);
	cout << endl;
	
	RSA_public_decrypt(EVP_MAX_MD_SIZE, sig, decrypt, pub, RSA_PKCS1_PADDING);
